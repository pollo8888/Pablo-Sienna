    ...
    <head>
        ...
        <style>
            ...
            .menu-button-respaldo {
                ...
                transition: right 0.3s ease;  /* Agregamos la transición */
            }
            .menu-button {
                position: fixed;
                top: 50%;
                right: 0px;
                transform: translateY(-50%);
                height: 17%; /* Altura del botón basada en la imagen */
                border: none;
                ...
                justify-content: center;
                align-items: center;
                outline: none; /* Eliminar el contorno alrededor del botón al hacer clic */
                display: flex;
            }
            /* Eliminar el contorno en el foco y el estado activo */
            .menu-button:focus,
            .menu-button:active {
                outline: none; /* Asegura que no haya contorno cuando se hace clic o se enfoca */
                box-shadow: none; /* También eliminar cualquier sombra que aparece con el foco */
            }
            /* Estilo para la imagen del icono */
            .menu-icon {
                width: 90%; /* Ajusta la imagen al tamaño del botón */
                height: 90%; /* Ajusta la imagen al tamaño del botón */
                object-fit: contain; /* Asegura que la imagen no se deforme */
            }
            .menu-button.open {
                right: 358px; /* Mueve el botón al ancho del menú */
            }
            ...
            .sidebar-lateral {
                position: fixed;
                top: 0;
                right: -360px; /* Oculto fuera de pantalla inicialmente */
                width: 360px;
                height: 100%;
                ...
                display: flex; /* Utiliza flexbox para manejar el contenido */
...
.sidebar-content {
    ...
    box-sizing: border-box; /* Evita desbordamiento */
            ...
            .timeline-item {
                position: relative;
                margin: 10px 0; /* Reduce el margen entre ítems */
                width: 100%; /* Ajusta el ancho para pegarlo a las esquinas */
                padding: 15px;
                border-radius: 8px;
                background: #fff;
...
.user-row {
    display: flex;
    justify-content: space-between; /* Espaciado entre usuario y botones */
...
.action-buttons i {
    pointer-events: none; /* Desactiva los eventos en el ícono para que funcionen en el botón */
}

.edit-button-tracing {
    outline: none; /* Elimina el cuadro de enfoque */
    border: none;  /* Elimina el borde (si es necesario) */
    background: none; /* Elimina el fondo (si es necesario) */
    padding: 0; /* Ajusta el padding para que se vea limpio */
    cursor: pointer; /* Cambia el cursor a pointer */
}
.edit-button-tracing:focus {
    outline: none; /* Asegúrate de que al recibir el foco no se muestre nada */
}
            ...
            .timeline-item.author-highlight {
                border-left: 4px solid #3498db; /* Color azul */
                padding-left: 10px; /* Espaciado para separar del borde */
                background-color: #f0f8ff; /* Fondo ligero opcional */
            }
            /* Estilo para otros usuarios */
            .timeline-item.other-highlight {
                border-left: 4px solid #e67e22; /* Color naranja delgado */
                padding-left: 10px;
                background-color: #fffaf0; /* Fondo ligero opcional */
            }
            /*-----*/
            #loading {
                font-size: 16px;
            ...
            .edit-folder-btn {
                position: absolute;
                right: 20px; /* Aumenta este valor para más separación */
                top: 50%;
                transform: translateY(-50%);
            ...
            .edit-folder-btn:focus {
                outline: none; /* Sin efecto de enfoque */
                box-shadow: none; /* Evita sombras adicionales */
            }
            /* Cambio de color al pasar el mouse (opcional) */
            .edit-folder-btn:hover {
                color: #333; /* Color más oscuro en hover */
            }

            /* Estilo del contenedor */
.input-container {
    position: sticky; /* Fija el contenedor dentro del menú lateral */
    bottom: 0; /* Coloca en la parte inferior */
    left: 0;
    ...
    z-index: 10; /* Asegura que esté por encima de los seguimientos */
}
    ...
    <div id="notification-badge" class="notification-badge">0</div>
        ...
        <div id="sidebar-lateral" class="sidebar-lateral">
                ...
                <?php } else { ?>
                    <ul id="timeline">
                        <?php foreach ($tracingsFolder as $tracing) : ?>
                            <li class="timeline-item <?php echo ($_SESSION['user']['id_user'] == $tracing['id_user_tracing']) ? 'author-highlight' : 'other-highlight'; ?>">
                            <div class="user-row">
                                ...
                                <?php if ($_SESSION['user']['id_user'] == $tracing['id_user_tracing']) : ?>
                                    <div class="action-buttons">
                                        ...
                                        <form action="#" method="POST">
                                            <input name="deleteTracing[id_tracing]" type="text" class="form-control" id="id_tracing" value="<?php echo $tracing['id_tracing']; ?>" readonly hidden style="display: none;">
                                            <input name="deleteTracing[key_tracing]" type="text" class="form-control" id="key_tracing" value="<?php echo $tracing['key_tracing']; ?>" readonly hidden style="display: none;">
                                        ...
                                        <form action="#" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar el seguimiento?');">
                                            <input name="deleteTracing[id_tracing]" type="text" class="form-control" id="id_tracing" value="<?php echo $tracing['id_tracing']; ?>" readonly hidden style="display: none;">
                                            <input name="deleteTracing[key_tracing]" type="text" class="form-control" id="key_tracing" value="<?php echo $tracing['key_tracing']; ?>" readonly hidden style="display: none;">
                                            <button class="btn delete-button" name="action" value="deleteTracing" id="btn-delete-tracing">
            ...
            <form action="#" method="post" class="input-container">
                <input name="dataTracing[id_folder_tracing]" type="text" class="form-control" required value="<?php echo $folder['id_folder']; ?>" readonly style="display: none;" hidden>
                <input name="dataTracing[id_user_tracing]" type="text" class="form-control" required value="<?php echo $_SESSION['user']['id_user']; ?>" readonly style="display: none;" hidden>
                <input name="dataTracing[key_tracing]" type="text" class="form-control" required value="TRC-<?php echo $clave; ?>" readonly style="display: none;" hidden>                            
                <input name="dataTracing[comment_tracing]" type="text" class="new-tracing-input" required placeholder="Seguimiento" autocomplete="off">
                
                <i class="fas fa-ellipsis-v modal-icon" data-toggle="modal" data-target="#modalAddTracing" id="ellipsis-icon" style="pointer-events: none; opacity: 0.5;"></i>
                                ...
                                <form action="#" method="post">
                                    <div class="row">
                                        <div class="col-lg-6 col-md-12 col-sm-12 position-relative">
                                            ...
                                            <?php if($_SESSION['user']['id_type_user'] == 1){ ?>
                                                <button type="button" class="edit-folder-btn" data-toggle="modal" data-target="#editFolderModal">
                                                    <i class="fas fa-pen"></i>
                                                </button>
                                            <?php } ?>
                                        </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <?php if($folder['fk_folder'] == 0){ ?>
                                            <a href="folders.php" class="btn btn-block" 
                                        ...
                                        <?php
                                            ...
                                            <a href="subfolder.php?id=<?php echo $folder['fk_folder']; ?>&key=<?php echo $keyFolder['key_folder']; ?>" class="btn btn-block" 
                                                style="background-color: #FF5800;
                                                color: #ffffff;
                                                text-decoration: none;
                                                display: inline-block;
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-4">
                                                    <?php if(empty($folders) AND empty($folderDocuments)) { ?>
                                                        <input type="text" class="form-control" style="margin-bottom:10px;" id="searchInputFolders" placeholder="Buscar expediente...">
                                                    <?php } else if(!empty($folders) AND empty($folderDocuments)) { ?>
                                                        <input type="text" class="form-control" style="margin-bottom:10px;" id="searchInputFolders" placeholder="Buscar expediente...">
                                                    <?php } else if(empty($folders) AND !empty($folderDocuments)) { ?>
                                                        <input type="text" class="form-control" style="margin-bottom:10px;" id="searchInputDocs" placeholder="Buscar documentos...">
                                                    <?php } else { ?>
                                                        <input type="text" class="form-control" style="margin-bottom:10px;" id="searchInputFolders" placeholder="Buscar expediente...">
                                                    <?php } ?>
                                                </div>
                                                ...
                                                <?php if($_SESSION['user']['id_type_user'] == 1 || $_SESSION['user']['id_type_user'] == 3){ ?>
                                                    <div class="col-lg-4">
                                                        ...
                                                        <a href="#" class="btn btn-block" style="background-color: blue; color: #ffffff;" role="button" aria-pressed="true" data-toggle="modal" data-target="#modalAgregarFullDocuments">
                                                            <i class="fas fa-plus pr-2"></i>Agregar documentos
                                                        </a>
                                                    </div>
                                                    <div class="col-lg-4 text-right">
                                                        <!-- Botón para abrir el modal de agregar nuevas carpetas -->
                                                        <a href="#" class="btn btn-block" style="background-color: blue; color: #ffffff;" role="button" aria-pressed="true" data-toggle="modal" data-target="#modalAgregarCarpeta">
                                            ...
                                            <?php if(empty($folders) AND empty($folderDocuments)) { ?>
                                                <div class="alert alert-info" role="alert">
                                                    <i class="fas fa-exclamation-triangle"></i>&nbsp;¡No se hallaron registros de expedientes para este cliente!
                                                </div>
                                            <?php } else { ?>
                                                <div class="row">
                                                            ...
                                                            <div class="folder">
                                                                <div class="title-bar" style="background-color: #f5f5f5; color: #000000; border-radius: 10px; padding: 5px; display: flex; flex-direction: column; align-items: stretch;">
                                                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                                                        <div class="title">
                                                                            <a href="subfolder.php?id=<?php echo $folder['id_sub_folder']; ?>&key=<?php echo $folder['key_sub_folder']; ?>" style="text-decoration: none; color: inherit;">
                                                                        ...
                                                                        <?php if($_SESSION['user']['id_type_user'] == 1){ ?>
                                                                            <div class="dropdown" style="margin-top:5px;">
                                                                                <button class="btn btn-secondary" type="button" id="dropdownMenuButton_<?php echo $folder['id_sub_folder']; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background-color: transparent; border: none;">
                                                                                    <i class="fas fa-ellipsis-v" style="color: black; background-color: transparent;"></i>
                                                                                </button>
                                                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton_<?php echo $folder['id_sub_folder']; ?>">
                                                                                    <a class="dropdown-item" href="#" data-folder-id="<?php echo $folder['id_sub_folder']; ?>">
                                                                                    ...
                                                                                    <?php if($_SESSION['user']['id_type_user'] == 1){ ?>
                                                                                        <hr>
                                                                                        <form action="#" method="POST">
                                                                                            <input name="deleteFolder[idFolder]" type="hidden" class="form-control form-control-sm" id="id_sub_folder" value="<?php echo $folder['id_sub_folder']; ?>" readonly hidden style="display: none;">
                                                                                            <button class="dropdown-item" type="submit" name="action" value="deleteFolder" onclick="return confirm('¿Estás seguro de eliminar el expediente?');">
                                                        ...
                                                        <div class="col-lg-3 col-md-4 col-sm-16" id="myDocuments">
                                                            <div class="card" style="width: 100%;">
                                                                <div class="card-body">
                                                                    ...
                                                                    <embed src="extensions/view_pdf.php?folder=<?php echo urlencode($folderDocument['key_folder']); ?>&file=<?php echo urlencode($folderDocument['file_name_document']); ?>" width="100%" height="300px" type="application/pdf">
                                                                    ...
                                                                    <div class="title-bar" style="display: flex; justify-content: space-between; align-items: center;">
                                                                        <div class="title">
                                                                            ...
                                                                            if($folderDocument['file_extension_document'] === 'pdf' || $folderDocument['file_extension_document'] === 'PDF'){ ?>
                                                                                <!--CÓDIGO NUEVO-->
                                                                                <a href="extensions/open_pdf.php?folder=<?php echo urlencode($folderDocument['key_folder']); ?>&file=<?php echo urlencode($folderDocument['file_name_document']); ?>" style="width: 40px; height: 40px;" target='_blank' class="btn btn-danger"><i class="fas fa-file-pdf"></i></a>
                                                                                <!--// CÓDIGO ANTIGUO - DESARROLLO EN UN SERVER-->
                                                                            ...
                                                                            //CUANDO ES UN WORD
                                                                                ...
                                                                                <!--CÓDIGO NUEVO-->
                                                                                <a href="extensions/download_pdf.php?folder=<?php echo urlencode($folderDocument['key_folder']); ?>&file=<?php echo urlencode($folderDocument['file_name_document']); ?>" download style="width: 40px; height: 40px;" class="btn btn-primary"><i class="fas fa-file-word"></i></a>
                                                                                <!--// CÓDIGO ANTIGUO - DESARROLLO EN UN SERVER-->
                                                                        ...
                                                                        <div class="dropdown">
                                                                            <button class="btn btn-secondary" type="button" id="dropdownMenuButton_<?php echo $folderDocument['id_document']; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background-color: transparent; border: none;">
                                                                                <i class="fas fa-ellipsis-v" style="color: black; background-color: transparent;"></i>
                                                                            </button>
                                                                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton_<?php echo $folderDocument['id_document']; ?>">
                                                                                <!--NUEVO CÓDIGO / MUESTRA EL DOCUMENTO EN OTRA VENTANA-->
                                                                                <?php 
                                                                                // CUANDO ES UN PDF
                                                                                if($folderDocument['file_extension_document'] === 'pdf' || $folderDocument['file_extension_document'] === 'PDF'){ ?>
                                                                                    <!--CÓDIGO NUEVO-->
                                                                                    <a class="dropdown-item" href="extensions/open_pdf.php?folder=<?php echo urlencode($folderDocument['key_folder']); ?>&file=<?php echo urlencode($folderDocument['file_name_document']); ?>" target='_blank'>
                                                                                    <!--// CÓDIGO ANTIGUO - DESARROLLO EN UN SERVER-->
                                                                                ...
                                                                                <!--CÓDIGO NUEVO-->
                                                                                <a class="dropdown-item" href="extensions/download_pdf.php?folder=<?php echo urlencode($folderDocument['key_folder']); ?>&file=<?php echo urlencode($folderDocument['file_name_document']); ?>" download>
                                                                                <!--// CÓDIGO ANTIGUO - DESARROLLO EN UN SERVER-->
                                                                                ...
                                                                                <?php if($_SESSION['user']['id_type_user'] == 1){ ?>
                                                                                    <hr>
                                                                                    <form action="#" method="POST">
                                                                                        <input name="deleteDocument[id_document]" type="text" class="form-control form-control-sm" id="id_document" value="<?php echo $folderDocument['id_document']; ?>" readonly hidden style="display: none;">
                                                                                        <input name="deleteDocument[key_document]" type="text" class="form-control form-control-sm" id="key_document" value="<?php echo $folderDocument['key_document']; ?>" readonly hidden style="display: none;">
                    ...
                    <div class="modal-body">
                        ...
                        <?php if($_SESSION['user']['id_type_user'] == 1 || $_SESSION['user']['id_type_user'] == 3){ ?>
                            ...
                            <form id="formAgregarCarpeta" action="#" method="POST">
                                <input name="newFolder[id_user_folder]" type="hidden" class="form-control" id="id_user_folder" required value="<?php echo $_SESSION['user']['id_user']; ?>" readonly style="display:none;" hidden>
                                <input name="newFolder[fk_folder]" type="hidden" class="form-control" id="fk_folder" required value="<?php echo $folder['id_folder']; ?>" readonly style="display:none;" hidden>
                                <input name="newFolder[key_folder]" type="hidden" class="form-control" id="key_folder" required value="CARP-<?php echo $clave; ?>" readonly style="display:none;" hidden>
                                
                                <div class="form-group">
                                    <label for="name_folder">Nombre del expediente:</label>
                                    <input type="text" name="newFolder[name_folder]" class="form-control" id="name_folder" required autocomplete="off">
                                </div>
                                ...
                                <?php if($_SESSION['user']['id_type_user'] == 1){ ?>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6 col-md-6 col-sm-12">
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input" id="opcion1" value="Si" name="newFolder[chk_alta_fact_folder]">
                                                <label class="form-check-label" for="opcion1">Vo.Bo. Alta Facturación</label>
                                            </div>
                                        </div>
                                        <div class="col-lg-6 col-md-6 col-sm-12">
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input" id="opcion2" value="Si" name="newFolder[chk_lib_folder]">
                                    ...
                                    <div class="row mt-2">
                                        <div class="col-lg-6 col-md-6 col-sm-12">
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input" id="opcion3" value="Si" name="newFolder[chk_orig_recib_folder]">
                    ...
                    <div class="modal-body">
                        ...
                        <?php if($_SESSION['user']['id_type_user'] == 1 || $_SESSION['user']['id_type_user'] == 3){ ?>
                            ...
                            <form id="formEditarCarpeta" action="#" method="POST">
                                <input type="hidden" name="updateFolder[id_folder]" id="edit_folder_id" style="display:none;" hidden readonly>
                                <div class="form-group">
        ...
        <div class="modal fade" id="modalAgregarDocuments" tabindex="-1" aria-labelledby="modalAgregarDocumentsLabel" aria-hidden="true">
            <div class="modal-dialog" style="max-width: 50%;">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalAgregarDocumentsLabel">Agregar documentos</h5>
                        &nbsp;&nbsp;&nbsp;
                        <button type="button" id="agregarDocumentoBtn" class="btn" style="background-color: gray; color: #ffffff; border-radius:40px; margin-top:-5px;"><i class="fas fa-plus"></i></button>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Cerrar">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form id="formAgregarDocuments" action="#" method="POST" enctype="multipart/form-data">
                            ...
                            <button type="submit" class="btn btn-lg btn-block" style="background-color: #37424A; color: #ffffff;" name="action" value="saveDocuments">Guardar documentos</button>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <div class="modal fade" id="modalAgregarFullDocuments" tabindex="-1" aria-labelledby="modalAgregarFullDocumentsLabel" aria-hidden="true">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="mb-3 files color">
                                            <input type="file" class="form-control" id="miarchivo" name="miarchivo[]" multiple="" required accept=".pdf">
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="modalAddTracing" tabindex="-1" aria-labelledby="modalAddTracingLabel" aria-hidden="true">
            <div class="modal-dialog" style="max-width: 50%;">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form id="addFormTracing" action="#" method="POST">
                            <input name="tracing[id_folder_tracing]" type="text" class="form-control" id="id_folder_tracing" required value="<?php echo $folder['id_folder']; ?>" readonly style="display: none;" hidden>
                            <input name="tracing[key_folder_tracing]" type="text" class="form-control" id="key_folder_tracing" required value="<?php echo $folder['key_folder']; ?>" readonly style="display: none;" hidden>
                            <input name="tracing[id_user_tracing]" type="text" class="form-control" id="id_user_tracing" required value="<?php echo $_SESSION['user']['id_user']; ?>" readonly style="display: none;" hidden>
                            <input name="tracing[key_tracing]" type="text" class="form-control" id="key_tracing" required readonly style="display: none;" hidden>
                            <input name="tracing[key_user]" type="text" class="form-control" id="key_user" required value="<?php echo $_SESSION['user']['key_user']; ?>" readonly style="display: none;" hidden>
                            ...
                            <div class="form-group">
                                ...
                                <button type="button" id="selectAllBtn" class="btn btn-primary btn-sm mt-2">Seleccionar Todo</button>
                                <button type="button" id="deselectAllBtn" class="btn btn-secondary btn-sm mt-2">Desmarcar Todo</button>
                            </div>
                            
                            <div class="form-group">
                                <label>Comentario:</label>
                                <textarea name="tracing[comment_tracing]" class="form-control" id="comment_tracing" rows="4" required></textarea>
                            </div>
                            
                            <div class="form-group">
                                <label for="send_notification" style="display: flex; align-items: center; font-size: 16px;">
                                    ¿Enviar notificación por correo electrónico?
                                    <input type="checkbox" id="send_notification" class="checkbox-custom" style="margin-left: 10px; transform: scale(1.5);">
                                </label>
                            </div>
                            ...
                            */ ?>
                            ...
                            <button type="submit" class="btn btn-lg btn-block" style="background-color: #37424A; color: #ffffff;" name="action" value="create" id="createNoticeBtn">Guardar</button>
                        </form>
        ...
        <div class="modal fade" id="modalEditTracing" tabindex="-1" aria-labelledby="modalEditTracingLabel" aria-hidden="true">
            <div class="modal-dialog" style="max-width: 50%;">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form id="editFormTracing" action="#" method="POST">
                            <input name="editTracing[id_tracing]" type="text" class="form-control" id="edit_id_tracing" required readonly style="display: none;" hidden>
                            <input name="editTracing[key_tracing]" type="text" class="form-control" id="edit_key_tracing" required readonly style="display: none;" hidden>
                            
                            <div class="form-group">
                                <label>Comentario:</label>
                                <textarea name="editTracing[comment_tracing]" class="form-control" id="edit_comment_tracing" rows="7" required></textarea>
                            </div>
                            
                            
                            <button type="submit" class="btn btn-lg btn-block" style="background-color: #37424A; color: #ffffff;" name="action" value="updateTracing" id="updateTracingBtn">Actualizar</button>
                        </form>
